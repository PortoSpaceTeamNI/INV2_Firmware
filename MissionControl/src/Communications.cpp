...
int write_to_rs485(uint8_t *buffer, size_t size)
{
    ...
    digitalWrite(RS_TX_ENABLE_PIN, RS485_TX_ENABLE_LEVEL); // switch to transmit mode
    if (Serial2.write(buffer, size) != size)
    {
        digitalWrite(RS_TX_ENABLE_PIN, RS485_RX_ENABLE_LEVEL); // switch back to receive mode
        return -1;
    }
    Serial2.flush();
    digitalWrite(RS_TX_ENABLE_PIN, RS485_RX_ENABLE_LEVEL); // switch back to transmit mode
    return 0;
}
...
/* Radio */
// Extern RadioLib radio object from Radio.cpp
extern SX1281 radio;
...
int read_from_radio(uint8_t *read_byte, packet_t *packet, cmd_parse_state_t *state)
{
    // Static buffer to hold radio RX data and track consumption
    static uint8_t rxBuffer[MAX_PACKET_SIZE] = {0};
    static size_t rxIndex = 0;
    static size_t rxSize = 0;

    // Try to read new data from radio if buffer is empty
    if (rxIndex >= rxSize) {
        String str;
        int err = radio.readData(str);
        if (err == RADIOLIB_ERR_NONE && str.length() > 0) {
            // Copy RadioLib string data into our buffer
            rxSize = str.length();
            if (rxSize > MAX_PACKET_SIZE) rxSize = MAX_PACKET_SIZE;
            memcpy(rxBuffer, str.c_str(), rxSize);
            rxIndex = 0;
        } else {
            // No data available from radio yet
            return 0;
        }
    }

    // Feed buffered bytes to state machine one at a time
    while (rxIndex < rxSize && *state != END) {
        *read_byte = rxBuffer[rxIndex++];
        *state = parse_input(*read_byte, packet, *state);
    }
...
packet_t *read_packet(int *error, interface_t interface)
{
    ...
    {
    case RADIO_INTERFACE:
        if (read_from_radio(&read_byte, packet, state) != 0)
        {
            return NULL;
        }
        break;
    ...
    *end = clock();
    ...
    if (*state != SYNC && msec > RS485_RESPONSE_TIMEOUT_MS) // timeout
    {
        *state = SYNC;
