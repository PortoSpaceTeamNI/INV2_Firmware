...
int valves_setup(data_t *data)
{
    ...
    servo.attach(PWM_SIG_PIN, SERVO_OPEN_PULSE_WIDTH, SERVO_CLOSE_PULSE_WIDTH);
...
void valve_set(data_t *data, uint8_t valve, uint8_t state)
{
    ...
    case VALVE_SERVO:
        ...
        if (state)
        {
            servo.writeMicroseconds(SERVO_OPEN_PULSE_WIDTH);
        }
        else
        {
            servo.writeMicroseconds(SERVO_CLOSE_PULSE_WIDTH);
        }
        delay(15); 
        break;
    default:
