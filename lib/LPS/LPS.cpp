...
LPS::LPS(void)
{
  ...
  _wire = &Wire; // default to Wire, but can be changed in init() if needed
}

// Public Methods ////////////////////////////////////////////////////

// sets or detects device type and slave address; returns bool indicating success
bool LPS::init(deviceType device, byte sa0, TwoWire* wire)
{
  _wire = wire;
...
int32_t LPS::readPressureRaw(void)
{
  ...
  if (_device == device_25H || _device == device_331AP)
  {
    ...
    _wire->write(PRESS_OUT_XL | (1 << 7));
  }
  else // 22DF
  {
    _wire->write(PRESS_OUT_XL);
  }
  _wire->endTransmission();
...
int16_t LPS::readTemperatureRaw(void)
{
  ...
  {
    // assert MSB to enable register address auto-increment
    _wire->write(TEMP_OUT_L | (1 << 7));
  }
  else // 22DF
  {
    _wire->write(TEMP_OUT_L);
  }
  _wire->endTransmission();
...
{
  // Try reading WHO_AM_I twice in case first read wakes the device
  _wire->beginTransmission(address);
  _wire->write(WHO_AM_I);
